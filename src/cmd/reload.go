...
the Mattermost connection and the listener against the new configuration —
all inside the same process. This is the intended way to pick up a "msg-gw
rules add/remove" change (or any other config.json edit) when the daemon was
started with "exec" as a container's entrypoint, where there is no
supervisor around to restart it for you.
...
keeps running on the configuration it already had. Sending the signal only
proves the daemon received it — check its logs for "reload:" to see whether
the new configuration was actually accepted.`,
	Args:         cobra.NoArgs,
	SilenceUsage: true,
