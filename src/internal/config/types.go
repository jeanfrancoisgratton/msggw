...
type GMessagesConfig struct {
	...
	// the first time it is bridged, regardless of how old they are. 0
	// disables it — a newly bridged conversation starts empty.
	//
	// Backfill posts everything it fetches — including attachments —
	// synchronously, before the bridge handles anything else, so a long count
	// on a conversation with a lot of media will visibly stall live traffic
