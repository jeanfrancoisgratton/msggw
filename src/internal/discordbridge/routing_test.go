...
// Original filename: src/internal/discordbridge/routing_test.go
...
func guildConversation(channelID, guildID, name string) discord.Conversation {
	return discord.Conversation{ID: channelID, GuildID: guildID, Name: name, IsGroup: true}
}

func dmConversation(channelID string, others ...string) discord.Conversation {
	conv := discord.Conversation{ID: channelID}
	for i, name := range others {
		conv.Participants = append(conv.Participants, discord.Participant{
...
func TestDiscordRouteFallsBackToDefault(t *testing.T) {
	router, err := NewRouter(config.DiscordRoutingConfig{DefaultDirect: defaultDest})
	if err != nil {
...
func TestDiscordRouteMatchesChannelID(t *testing.T) {
	...
	router, err := NewRouter(config.DiscordRoutingConfig{
...
func TestDiscordRouteMatchesGuildID(t *testing.T) {
	...
	if dest, _ := router.Route(guildConversation("c2", "guild-2", "general")); !sameDest(dest, defaultDest) {
...
func TestDiscordRouteUsesDefaultGroupWhenSet(t *testing.T) {
	...
	if dest, _ := router.Route(dmConversation("c1", "Alice")); !sameDest(dest, defaultDest) {
...
func TestDiscordRouteShapeFilterNarrowsIdentityCriteria(t *testing.T) {
	...
	if dest, _ := router.Route(guildConversation("g2", "guild-2", "general")); !sameDest(dest, defaultDest) {
...
func TestDiscordRouteChannelNamePatternMatchesTitle(t *testing.T) {
	...
	if dest, _ := router.Route(guildConversation("c2", "guild-1", "general")); !sameDest(dest, defaultDest) {
...
func TestNewDiscordRouterRejectsABadPattern(t *testing.T) {
	_, err := NewRouter(config.DiscordRoutingConfig{
		DefaultDirect: defaultDest,
		Rules: []config.DiscordRule{{
			Name:               "broken",
			ChannelNamePattern: "([",
